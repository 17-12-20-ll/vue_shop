          ...
          <div :class="{on:loginWay}">
            <section class="login_message">
              ...
              <button :disabled="!rightPhone || computeTime>0" class="get_verification"
                ...
                {{computeTime?`重新发送(${computeTime}s)`:'获取验证码'}}
              </button>
            </section>
          ...
          <div :class="{on:!loginWay}">
            <section>
              ...
              <section class="login_message">
                ...
                <img class="get_verification" ref="captcha" src="http://localhost:4000/captcha"
                     alt="captcha"
                     @click="getCaptcha">
              </section>
            </section>
...
<script>
    ...
    //在组件中发送请求
    import {getSendCode, postSmsLogin, postPwdLogin} from '../../api'
    ...
    export default {
        ...
        methods: {
            // 异步获取短信验证码
            async getCode() {
                //如果当前没有计时,才允许进行计时处理，发送请求也只能在没有计时的时候发送
                if (!this.computeTime) {
                    //启动倒计时
                    this.computeTime = 30
            ...
            async login() {
                ...
                if (this.loginWay) {//短信登录
                    ...
                    // 发送ajax请求 进行短信登录
                    result = await postSmsLogin(phone, code)
                } else {//密码登录
                    ...
                    // 发送ajax请求，进行密码登录
                    result = await postPwdLogin({name, pwd, captcha})
                }
                // 停止计时器
                if (this.computeTime) {
                    this.computeTime = 0
